...
public class BarChartRenderer: ChartDataRendererBase
{
    ...
    internal func prepareBarHighlight(x x: CGFloat, y: Double, barspacehalf: CGFloat, from: Double, trans: ChartTransformer, inout rect: CGRect)
    {
        let barWidth: CGFloat = 0.5;
        
        let left = x - barWidth + barspacehalf;
        let right = x + barWidth - barspacehalf;
        let top = y >= from ? CGFloat(y) : CGFloat(from);
        let bottom = y <= from ? CGFloat(y) : CGFloat(from);
    ...
    public override func drawHighlighted(context context: CGContext, indices: [ChartHighlight])
    {
        ...
        for (var i = 0; i < indices.count; i++)
        {
            ...
            if (CGFloat(index) < (CGFloat(delegate!.barChartRendererChartXMax(self)) * _animator.phaseX) / CGFloat(setCount))
            {
                ...
                let y = isStack ? e.values[h.stackIndex] + e.getBelowSum(h.stackIndex) : e.value;
                
                // this is where the bar starts
                let from = isStack ? e.getBelowSum(h.stackIndex) : 0.0;
                ...
                if (drawHighlightArrowEnabled)
                {
                    ...
                    let xToYRel = abs(onePoint.y / onePoint.x);
